...
async function fetchLinhasPrecificacao(): Promise<LinhaPrecificacao[]> {
  ...
  return (data ?? []).map((r: Record<string, unknown>) => ({
    ...
    taxaCartao: Number(r.taxa_cartao) ?? 0,
...
// ---- Usuários de empresa (já carregados em login-unificado; aqui só complementamos se store vazio) ----
// Não sobrescrevemos usuariosEmpresa aqui para não apagar quem logou; carregarUsuariosEmpresaFromSupabase faz isso na tela de login.
  ...
  try {
    ...
    updateStore((prev) => {
      ...
      if (!parametrosCusto.empresaId && empresas.length > 0) {
        parametrosCusto = { empresaId: empresas[0].id, totalPecasEstoque: 0, descontoAVistaFixo: 5 }
      }
      return {
        ...prev,
        empresas,
